...
def is_pure_positive(comment: str, nps_score: Optional[int] = None) -> bool:
    ...
    comp_triggers = ["roi", "interest", "charges", "deviation", "paisa", "matlab", "call", "down payment", "मतलब", "कोई मतलब", "लोन लेने", "कॉल पर"]
...
def detect_gibberish(text: str) -> int:
    import re
    # FIRST VALIDATION: Less than 10 characters is automatically gibberish
    ...
    # 0. Check for complaints about physical products (we do loans, not cars/phones)
    physical_products = ["car", "bike", "motorcycle", "vehicle", "engine", "tyre", "tire", "mobile", "phone display", "screen broken", "tv", "refrigerator"]
    if any(p in text_lower for p in physical_products) and any(w in text_lower for w in ["problem", "issue", "not working", "broken", "repair", "service", "defect"]):
        return 1
    ...
    single_generic_words = [
        "good", "bad", "ok", "okay", "nice", "poor", "fine", "yes", "no",
        "super", "best", "worst", "better", "great", "thanks", "thank", "accha", "badiya"
    ]
...
def redact_pii(text: str) -> str:
    ...
    # Redact Loan/Account Numbers (numeric sequences 8 digits or longer)
...
def fix_sentiment_priority_text(
    ...
    positive_words = [
        ...
        "satisfied", "love", "helpful", "quick", "fast",
        "smooth", "easy", "thank", "thanks", "awesome", "amazing",
        "wonderful", "fantastic", "superb", "brilliant", "perfect",
    ]
    ...
    negative_signals = [
        "not good", "not proper", "not clear", "not updated", "not informed",
        "no information", "no one", "no update", "without information", "without prior",
        "too much", "extra charge", "wrong amount", "overcharged", "forced", "force",
        "mis-selling", "mis-sold", "fake promise", "coercion",
        "bribes", "bribe", "roi is high", "high roi", "interest rate", "deviation",
        "paisa", "matlab", "down payment", "not so good", "very high", "too high"
    ]
    ...
    if any(neg_phrase in text for neg_phrase in ["not good", "not happy", "not satisfied", "not clear", "not so good"]):
        has_positive = False
    ...
    import re
    callback_signals = ["please call", "call me", "callback", "call karne", "call karo", "call kijiye", "call lagata"]
    has_callback = any(re.search(r'\b' + re.escape(sig) + r'\b', text) for sig in callback_signals)

    critical_signals = [
        # English / Legal & Regulatory
        "rbi", "ombudsman", "consumer court", "consumer forum", "legal action",
        "legal notice", "lawyer", "advocate", "court case", "court", "police", "fir", "ipc",
        # Hinglish / Regional Transliterated Legal
        "case kar", "case karunga", "case karungi", "police me", "court jaunga", "rbi me",
        "legal warning", "court me", "police station",
        # Churn & Refusal to Pay EMI
        "close loan", "close my loan", "closing loan", "cancel loan", "cancel my loan",
        "transfer loan", "stop paying emi", "will not pay emi", "emi nahi dunga",
        "loan band", "loan band karo", "paisa nahi dunga", "kist nahi dunga", "emi stop",
        # Severe Harassment, Distress & Threats
        "threatened", "dhamki", "blackmail", "suicide",
        "mar jaunga", "mental torture", "threatened me", "agent abused", "abused me",
        # Fraud & Unauthorized Financial Loss
        "unauthorized debit", "unauthorized deduction", "double debit", "money stolen",
        "fraud", "scam", "cheating", "fake executive", "fake agent",
        # Native Devanagari Hindi Script
        "आरबीआई", "कंज्यूमर कोर्ट", "लीगल नोटिस", "पुलिस", "केस करूंगा", "केस करेंगे",
        "लोन बंद", "किस्त नहीं दूंगा", "पैसे नहीं दूंगा", "आत्महत्या", "मर जाऊंगा",
        "धमकी", "धोखाधड़ी", "ठगी", "रद्द"
    ]

    has_critical = False
    for signal in critical_signals:
        if re.search(r'\b' + re.escape(signal) + r'\b', text, re.IGNORECASE) or re.search(r'\b' + re.escape(signal) + r'\b', comment, re.IGNORECASE):
            has_critical = True
            break
    ...
    if has_critical:
        ...
        sentiment = "Negative"
        priority = "medium"
    elif has_positive and not has_negative and category not in ["Agent Behaviour", "Agent Misinformation & Mis-selling"]:
        sentiment = "Positive"
        priority = "low"

        if not observation or "concern" in observation.lower() or not observation.strip():
            observation = "The customer shared positive feedback about the service."

        if not recommendations or "concern" in recommendations.lower() or not recommendations.strip():
            recommendations = "Continue maintaining good service quality."

    elif has_negative:
        sentiment = "Negative"

    if sentiment.lower() == "negative" and priority == "low":
        if category.strip() not in ["Unwanted Communications", "Pricing & Charges"]:
            priority = "medium"
