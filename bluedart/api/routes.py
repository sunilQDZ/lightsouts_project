...
from bluedart.models.schemas import InferenceRequest, InsightResponse, InsightPredictionItem
from bluedart.core.security import require_api_key
from bluedart.services.insight_service import generate_insight
from bluedart.core.config import settings
from bluedart.services.batch_processor import run_batch_pipeline
...
log_file_path = "bluedart_app.log"
...
async def start_daily_analysis(background_tasks: BackgroundTasks):
    ...
    from Blue Dart survey data (e.g. Last Mile Delivery, Hub & Transit),
    ...
    return {
        "status": "success",
        "message": "Production Blue Dart daily detractor analysis started in background. Processing all day-1 detractors into AI_Analyzed and voc_alerts."
    }
...
async def clear_cache():
    ...
    from bluedart.services.insight_service import CACHE_FILE as cache_file
...
async def get_logs(lines: int = 1000):
    """
    Fetch the latest logs for bluedart.
    """
