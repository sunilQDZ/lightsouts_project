...
from bluedart.services.heuristics import (
...
from bluedart.core.database import load_categories_from_db
from bluedart.services.anthropic_client import (
    call_anthropic_llm
)
import json
import os
...
MAX_CACHE_SIZE = 5000
CACHE_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), "bluedart_inference_cache.json")
...
async def generate_insight(comment: str, item_id: str, touchpoint_name: str = "", nps_score: Optional[int] = None) -> Dict:
    ...
    if result.get("is_gibberish") == 1:
        ...
        result["status"] = "open"
    elif priority_val == "medium" or category_val == "Shipment Tracking":
        if category_val in ["Pricing & Charges", "COD Issues", "Returns & Refunds", "Documentation"]:
            result["status"] = "auto closed"
        else:
