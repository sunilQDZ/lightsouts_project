...
def handle_positive_feedback(comment, category, sentiment, priority, observation, recommendations, customer_response, confidence_score, is_gibberish=0, nps_score=None):
    ...
    comp_triggers = ["late", "delayed", "damaged", "missing", "stolen", "fraud", "fake", "call", "not received"]
...
def detect_gibberish(text: str) -> int:
    import re
    # FIRST VALIDATION: Less than MIN_COMMENT_LENGTH characters is automatically gibberish
    ...
    unwanted_words = ["n/a", "na", "nil", "nill", "none", "nothing", "test", "testing", "xxx", "000", "no", "nope", "nahi", "abc", "xyz", "qwerty", "asdf"]
    single_generic_words = [
        "good", "bad", "ok", "okay", "nice", "poor", "fine", "yes", "no",
        "super", "best", "worst", "better", "great", "thanks", "thank", "accha", "badiya",
        "average", "bakwas", "faltu", "bekar"
    ]
    two_word_generics = ["very bad", "very good", "too bad", "too good", "so bad", "so good", "not good", "not bad", "no comments", "no comment"]
    
    if len(words) <= 3 and any(w in unwanted_words for w in words):
        return 1
    if len(words) == 1 and words[0] in single_generic_words:
        return 1
    if text_lower in unwanted_words or text_lower.replace(".", "") in unwanted_words or text_lower in single_generic_words or text_lower in two_word_generics:
        return 1
        
    # 1.5 Check for low character entropy (spam/mashing like aaaaabbbb)
    # If a string is >= 10 chars but has less than 5 unique characters, it's likely gibberish
    alpha_only = re.sub(r'[\s\W_]', '', text_lower)
    if len(alpha_only) >= 10 and len(set(alpha_only)) <= 4:
        return 1
...
def fix_sentiment_priority_text(
    ...
    negative_words = [
        ...
        "security", "privacy", "hacked", "unsafe", "leak",
    ...
    negative_signals = [
        ...
        "missing", "stolen", "lost", "damaged", "broken", "fake delivery",
        "not delivered", "fraud", "cheating", "late delivery", "security issue", "data leak"
    ]
    ...
    critical_signals = [
        # Regulatory, Legal & Ombudsman
        "rbi", "ombudsman", "rbi complaint", "consumer court", "consumer forum", "legal action",
        "legal notice", "lawyer", "advocate", "court case", "court", "police", "fir", "ipc",
        # Hinglish / Regional Transliterated Legal
        "case kar", "case karunga", "case karungi", "police me", "court jaunga",
        "legal warning", "court me", "police station",
        # Package Loss & Severe Fraud
        "stolen", "package lost", "expensive item missing", "valuable missing", "theft",
        "delivery boy stole", "fake delivery signature",
        # Severe Harassment, Distress & Threats
        "harassment", "harassing", "threatened", "dhamki", "blackmail", "suicide",
        "mar jaunga", "mental torture", "threatened me", "agent abused", "abused me",
        # Fraud & Unauthorized Loss
        "unauthorized deduction", "double payment", "money stolen",
        "fraud", "scam", "cheating", "fake delivery agent", "emi nahi dunga",
        # Security & Privacy
        "data privacy", "account hacked", "safety concern", "data leak", "unsafe",
        # Native Devanagari Hindi Script
        "कंज्यूमर कोर्ट", "लीegal नोटिस", "पुलिस", "केस करूंगा", "केस करेंगे",
        "पार्सल चोरी", "सामान गायब", "आत्महत्या", "मर जाऊंगा",
        "धमकी", "परेशान कर रहे", "धोखाधड़ी", "ठगी"
    ]

    high_signals = [
        "fake delivery", "fake delivery attempt", "door locked status false",
        "rude agent", "rude delivery", "agent abused me", "demanded bribe",
        "extra cash demanded", "wrong address delivery", "delivered to wrong person"
    ]
    ...
    if has_critical:
        ...
        if sentiment != "Positive":
            sentiment = "Negative"
        priority = "medium"
    elif has_positive and not has_negative and category not in ["Agent Behaviour"]:
        sentiment = "Positive"
        priority = "low"
    ...
    # Escalation block removed per user request
